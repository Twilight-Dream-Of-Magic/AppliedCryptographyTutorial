...
#include "../../StreamDataCryption/ChinaShangYongMiMa/SM4.hpp"
...
auto main(int argument_cout, char* argument_vector[]) -> int
{
	...
	ChinaShangYongMiMa::SM4::TestSM4();
	...
	if constexpr(false)
	{
		...
		*/
		...
		constexpr std::uint32_t bb = b ^ std::rotl_uint32((a + d), 7);
		constexpr std::uint32_t cc = c ^ std::rotl_uint32((a + bb), 9);
		constexpr std::uint32_t dd = d ^ std::rotl_uint32((cc + bb), 13);
		constexpr std::uint32_t aa = a ^ std::rotl_uint32((cc + dd), 18);

		std::cout << "a:\n" << std::bitset<32>(a).to_string() << '\n';
		std::cout << "d:\n" << std::bitset<32>(d).to_string() << '\n';
		std::cout << "b:\n" << std::bitset<32>(b).to_string() << '\n';
		std::cout << "a + d:\n" << std::bitset<32>(a + d).to_string() << '\n';
		std::cout << "(a + d) <<< 7:\n" << std::bitset<32>(std::rotl_uint32((a + d), 7)).to_string() << '\n';
		...
		std::cout << "(a + bb) <<< 9:\n" << std::bitset<32>(std::rotl_uint32((a + bb), 9)).to_string() << '\n';
		...
		std::cout << "(cc + bb) <<< 13:\n" << std::bitset<32>(std::rotl_uint32((cc + bb), 13)).to_string() << '\n';
		...
		std::cout << "(cc + dd) <<< 18:\n" << std::bitset<32>(std::rotl_uint32((cc + dd), 18)).to_string() << '\n';
	...
	if constexpr(false)
	{
		...
		*/
		...
		constexpr std::uint32_t ddd = std::rotl_uint32(dd, 16);
		...
		constexpr std::uint32_t bbb = std::rotl_uint32(bb, 12);
		...
		constexpr std::uint32_t ddddd = std::rotl_uint32(dddd, 8);
		...
		constexpr std::uint32_t bbbbb = std::rotl_uint32(bbbb, 7);
